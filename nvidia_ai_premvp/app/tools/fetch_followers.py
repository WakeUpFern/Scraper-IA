"""
Implementación real de la herramienta fetch_followers_batch.
...
from app.tools.schemas import FetchFollowersInput, FetchFollowersOutput, FollowerInfo
...
class FetchFollowersTool(BaseTool[FetchFollowersInput, FetchFollowersOutput]):
    """Herramienta real fetch_followers_batch."""

    def __init__(self):
        super().__init__(
            name="fetch_followers_batch",
            description="Obtiene seguidores reales de una cuenta cargando cookies de sesión y guardándolos en base de datos.",
            input_model=FetchFollowersInput,
            output_model=FetchFollowersOutput
        )

    def _parse_network(self, responses: List[Response]) -> Optional[List[Dict[str, Any]]]:
        """Busca respuestas de red JSON con lista de seguidores."""
        for response in responses:
            url = response.url
            try:
                # Instagram followers API endpoint
                if "api/v1/friendships/" in url and "/followers/" in url:
                    data = response.json()
    ...
    def _parse_dom(self, page_source: Selector) -> List[Dict[str, Any]]:
        """DOM Fallback usando selectores de Scrapling para extraer seguidores de la lista renderizada."""
        results = []
    ...
    async def _run(self, id_analisis: int, id_usuario: int | None, input_data: FetchFollowersInput) -> FetchFollowersOutput:
        ...
        try:
            ...
            url = f"https://www.{input_data.platform}.com/{input_data.username}/followers/"
            ...
            for item in extracted:
                ...
                # Asegurar la identidad digital del seguidor
                id_follower = await db.ensure_digital_identity(
                    platform=input_data.platform,
                    username=username,
                    nombre_publico=full_name
                )
                
                # Guardar el vínculo en la base de datos real: id_follower -> sigue a -> id_target
                try:
                    await db.execute(
                        """
                        ...
                        """,
                        id_analisis, id_follower, id_target, tipo_vinculo_id, json.dumps(item)
                    )
                ...
                mock_db.add_vinculo_social({
                    ...
                    "target_username": input_data.username,
                    "type": "follows",
                    "weight": 1.0
                })
